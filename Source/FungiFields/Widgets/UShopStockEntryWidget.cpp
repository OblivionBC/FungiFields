...
void UShopStockEntryWidget::SetupEntry(AShopActor* InShopActor, const FShopStockEntry& InEntry,
...
{
	if (!CachedEntry.Item || !BuyPriceText)
	{
		return;
	}
...
{
	if (!ShopActor || !InventoryComp || !AbilitySystemComp || !CachedEntry.Item)
	{
		return;
	}

	const int32 Quantity = QuantitySpinner ? FMath::Max(1, FMath::RoundToInt(QuantitySpinner->GetValue())) : 1;
	ShopActor->TryBuyItem(InventoryComp, AbilitySystemComp, CachedEntry, Quantity);
}
